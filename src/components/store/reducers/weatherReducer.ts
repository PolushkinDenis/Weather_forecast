...
export const weatherReducer = (state = initialState, action: WeatherAction): WeatherState => {
    ...
    switch (action.type) {
        case WeatherActionTypes.FETCH_WEATHER_7DAYS:
            return { loading: true, error: null, weather7Days: [], weatherPast: state.weatherPast }
        case WeatherActionTypes.FETCH_WEATHER_PAST:
            return { loading: true, error: null, weather7Days: state.weather7Days, weatherPast: [] }
